...
export const getTodo = async (req: Request, res: Response) => {
  try {
    ...
    const todosWithLikes = todos.map((todo) => ({
      ...
      status: todo.status,
      user: todo.user
        ? { userId: todo.user.userId, userName: todo.user.userName }
        : null,
      likedBy: todo.likedBy
        ? todo.likedBy.map((u) => ({ userId: u.userId, userName: u.userName }))
        : [],
      comments: todo.comments
        ? todo.comments.map((c) => ({
            ...
            comment: c.comment,
            user: c.user
              ? { userId: c.user.userId, userName: c.user.userName }
              : null,
          }))
        : [],
    ...
    const todosById = {
      ...
      comments: todo.comments
        ? todo.comments.map((comment) => ({
            ...
            user: comment.user
              ? { userId: comment.user.userId, userName: comment.user.userName }
              : null,
          }))
        : [],
    };
...
export const addComment = async (req: Request, res: Response) => {
  const { comment, userId, taskId } = req.body as {
    comment: string;
    userId: number;
    taskId: number;
  };
  ...
  if (!comment || !userId) {
    return res
      .status(400)
      .json({ message: "Both comment and userId are required" });
  }
...
export const likeTodo = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const alreadyLiked = todo.likedBy.some(
      (people) => people.userId === user.userId
    );
    if (alreadyLiked) {
...
export const addTodo = async (req: Request, res: Response) => {
  try {
    const { content, status, userId } = req.body as {
      content: string;
      status: string;
      userId: number;
    };
  ...
  try {
    ...
    res.status(200).json({
      ...
      task: {
        ...
        user: {
          userId: taskToUpdate.user.userId,
          userName: taskToUpdate.user.userName,
        },
      },
    });
  } catch (error) {
